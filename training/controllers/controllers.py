...
class TodoController(http.Controller):
    ...
    def index(self, **kw):
        tasks = request.env['training.task'].sudo().search([])
        return request.render('training.halaman_tugas_ui', {'tasks': tasks})

    # --- API: CRUD sederhana ---

    @http.route('/api/tasks', auth='user', type='http', methods=['GET'], csrf=False)
    def get_all(self, limit=10, offset=0, **kw):
        tasks = request.env['training.task'].search([], limit=int(limit), offset=int(offset))
    ...
    def create(self, **kw):
        ...
        task = request.env['training.task'].create({'name': data['name'], 'description': data.get('description', '')})
    ...
    def update(self, task_id, **kw):
        task = request.env['training.task'].browse(task_id)
        if not task.exists():
